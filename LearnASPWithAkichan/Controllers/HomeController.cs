...
namespace LearnASPWithAkichan.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public bool KiemTraLopDaDangKy(string subject_id, string student_id)
        {
            ...
            List<PrerequisiteSubject> monTienQuyet = DanhSachMonTienQuyet(student_id);
            List<ClassSession> LopDaHoc = new List<ClassSession>();

            bool checker = false;

            //Xac dinh mon co bi trung hay khong (da dang ky chua)
            ...
            if (!checker)
            {
                foreach (var item in danhSachLopHP)
                {
                    foreach (var item2 in monTienQuyet)
                    {
                        if (item.Id == item2.SubjectId && item2.PrerequisiteSubjectId == null)
                        {
                            checker= false;
                        }
                        else
                        {
                            foreach (var item3 in monTienQuyet)
                            {
                                if (item3.PrerequisiteSubjectId == item.SubjectId)
                                {
                                    checker = true;
                                }
                                else
        ...
        public List<PrerequisiteSubject> DanhSachMonTienQuyet(string subject_id)
        {
            List<PrerequisiteSubject> subjects = new List<PrerequisiteSubject>();
            ...
            foreach (var item in lst_monTienQuyet)
            {
                ...
                {
                    foreach (var item2 in _db.PrerequisiteSubjects.Where(s => s.SubjectId == subject_id))
                    {
        ...
        public IActionResult ThongBao()
        {
            ...
            if (trongKyDangKy && RoleCheckByHand)
            {
                ...
                TempData["notification"] = "Môn ngoài kế hoạch";
